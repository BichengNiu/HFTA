...
def display_industrial_tabs(st_session_state, extract_industry_name_func):
    ...
    with st.sidebar:
        ...
        # 数据状态面板
        if st_session_state.get('preview_data_loaded_files'):
            st.markdown("---")
            st.markdown("**数据状态：**")
            
            # 显示指标数量
            weekly_count = len(st_session_state.get('preview_weekly_df', pd.DataFrame()).columns) if not st_session_state.get('preview_weekly_df', pd.DataFrame()).empty else 0
            monthly_count = len(st_session_state.get('preview_monthly_df', pd.DataFrame()).columns) if not st_session_state.get('preview_monthly_df', pd.DataFrame()).empty else 0
            daily_count = len(st_session_state.get('preview_daily_df', pd.DataFrame()).columns) if not st_session_state.get('preview_daily_df', pd.DataFrame()).empty else 0
            ...
            # 显示行业数量
            weekly_industries = len(st_session_state.get('preview_weekly_industries', []))
            monthly_industries = len(st_session_state.get('preview_monthly_industries', []))
    ...
    # 检测文件是否被删除（用户点击了叉号）
    if not uploaded_industrial_file and st_session_state.get('preview_data_loaded_files'):
        # 文件被删除，清空所有相关数据
        keys_to_clear = [
            # 主要数据
    ...
    if uploaded_industrial_file:
        ...
        if st_session_state.get('preview_data_loaded_files') != uploaded_file_name:
            with st.spinner('正在加载和处理工业数据...'):
                try:
                    ...
                    st_session_state.preview_weekly_df = weekly_df
                    st_session_state.preview_monthly_df = monthly_df
                    st_session_state.preview_daily_df = daily_df
                    st_session_state.preview_source_map = source_map
                    st_session_state.preview_indicator_industry_map = indicator_map
                    st_session_state.preview_data_loaded_files = uploaded_file_name
                    ...
                    st_session_state.preview_weekly_industries = sorted(list(weekly_clean_industries))
                    st_session_state.preview_monthly_industries = sorted(list(monthly_clean_industries))
                    st_session_state.preview_daily_industries = sorted(list(daily_clean_industries))
                    st_session_state.preview_clean_industry_map = clean_map
                    
                    st_session_state.preview_weekly_summary_cache = {}
                    st_session_state.preview_monthly_summary_cache = {}
                    st_session_state.preview_daily_summary_cache = {}
                    
                    if not monthly_df.empty:
                         st_session_state.preview_monthly_growth_summary_df = calculate_monthly_growth_summary(monthly_df)
                         monthly_industries_list = st_session_state.get('preview_monthly_industries', [])
                         current_monthly_df = st_session_state.preview_monthly_df
                         current_clean_industry_map_m = st_session_state.get('preview_clean_industry_map', {})
                         current_source_map_m = st_session_state.get('preview_source_map', {})
                         temp_monthly_cache = {}
                         if monthly_industries_list:
                            ...
                            for industry_name_m in monthly_industries_list:
                                ...
                                if industry_indicator_cols_m:
                                    ...
                                    temp_monthly_cache[industry_name_m] = pd.DataFrame()
                            st_session_state.preview_monthly_summary_cache = temp_monthly_cache
                            print(f"[Industrial Tab] Monthly summary cache populated with {len(temp_monthly_cache)} entries.")

                    # <<< 新增：在此处预计算周度行业摘要缓存 >>>
                    if not weekly_df.empty:
                        weekly_industries_list_for_cache = st_session_state.get('preview_weekly_industries', [])
                        current_weekly_df_for_cache = st_session_state.preview_weekly_df
                        current_clean_industry_map_w = st_session_state.get('preview_clean_industry_map', {})
                        current_source_map_w = st_session_state.get('preview_source_map', {})
                        temp_weekly_cache = {}
                        if weekly_industries_list_for_cache:
                            ...
                            with st.spinner("正在计算周度行业摘要以备总览..."): # Spinner for this specific pre-calc
                                for industry_name_w_cache in weekly_industries_list_for_cache:
                                    ...
                                    if industry_indicator_cols_w:
                                        ...
                                        temp_weekly_cache[industry_name_w_cache] = pd.DataFrame()
                                st_session_state.preview_weekly_summary_cache = temp_weekly_cache
                                print(f"[Industrial Tab] Weekly summary cache pre-populated with {len(temp_weekly_cache)} entries.")
                    # <<< 结束新增 >>>

                    # <<< 新增：在此处预计算日度行业摘要缓存 >>>
                    if not daily_df.empty:
                        daily_industries_list_for_cache = st_session_state.get('preview_daily_industries', [])
                        current_daily_df_for_cache = st_session_state.preview_daily_df
                        current_clean_industry_map_d = st_session_state.get('preview_clean_industry_map', {})
                        current_source_map_d = st_session_state.get('preview_source_map', {})
                        temp_daily_cache = {}
                        if daily_industries_list_for_cache:
                            ...
                            with st.spinner("正在计算日度行业摘要以备总览..."): # Spinner for this specific pre-calc
                                for industry_name_d_cache in daily_industries_list_for_cache:
                                    ...
                                    if industry_indicator_cols_d:
                                        ...
                                        temp_daily_cache[industry_name_d_cache] = pd.DataFrame()
                                st_session_state.preview_daily_summary_cache = temp_daily_cache
                    ...
                    st_session_state.preview_data_loaded_files = None
                    st_session_state.preview_weekly_df = pd.DataFrame()
                    st_session_state.preview_monthly_df = pd.DataFrame()
                    st_session_state.preview_daily_df = pd.DataFrame()
                    st_session_state.preview_monthly_summary_cache = {}
                    st_session_state.preview_weekly_summary_cache = {}
                    st_session_state.preview_daily_summary_cache = {}

    # <<< 将周度行业摘要的预计算移到这里 >>>
    if st_session_state.get('preview_data_loaded_files') and not st_session_state.get('preview_weekly_df', pd.DataFrame()).empty:
        current_weekly_df = st_session_state.preview_weekly_df
        current_weekly_industries = st_session_state.get('preview_weekly_industries', [])
        current_clean_industry_map_for_weekly = st_session_state.get('preview_clean_industry_map', {})
        current_source_map_for_weekly = st_session_state.get('preview_source_map', {})
        # 确保 preview_weekly_summary_cache 已初始化为空字典
        if 'preview_weekly_summary_cache' not in st_session_state or not st_session_state.preview_weekly_summary_cache:
            st_session_state.preview_weekly_summary_cache = {}
        
    data_is_ready = st_session_state.get('preview_data_loaded_files') is not None
    ...
    with tab_overall:
        ...
        if not data_is_ready or (st_session_state.get('preview_weekly_df', pd.DataFrame()).empty and st_session_state.get('preview_monthly_df', pd.DataFrame()).empty):
            st.info("请先上传数据文件以查看总体概览。")
        else:
            # --- 日度数据总览 ---
            st.markdown("#### 日度数据总览")
            daily_summary_cache_for_overview = st_session_state.get('preview_daily_summary_cache', {})
            preview_daily_df_exists = not st_session_state.get('preview_daily_df', pd.DataFrame()).empty
            ...
            # --- 周度数据总览 ---
            st.markdown("#### 周度数据总览")
            weekly_summary_cache_for_overview = st_session_state.get('preview_weekly_summary_cache', {})
            preview_weekly_df_exists = not st_session_state.get('preview_weekly_df', pd.DataFrame()).empty
            ...
            # --- 月度数据总览 ---
            st.markdown("#### 月度数据总览")
            monthly_summary_cache_for_overview = st_session_state.get('preview_monthly_summary_cache', {})
            preview_monthly_df_exists = not st_session_state.get('preview_monthly_df', pd.DataFrame()).empty
