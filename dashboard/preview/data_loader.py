    ...
    for file_input in excel_files_input:
        ...
        try:
            ...
            for sheet_name in sheet_names:
                ...
                header_row = 0 # Default for Wind and others
                skip_row_after_read = False # Flag to skip the row after header (for 同花顺)
                ...
                if "mysteel" in sheet_name.lower() or "myteel" in sheet_name.lower(): # Handle typo and case
                    ...
                    print(f"     Detected 'Mysteel/Myteel', using header row {header_row + 1}.")
                elif "同花顺" in sheet_name: # 同花顺 uses first row as header, but skip Indicator ID row later
                    header_row = 0
                    skip_row_after_read = True
                    print(f"     Detected '同花顺', using header row {header_row + 1} and will skip next row.")
                ...
                try:
                    ...
                    if skip_row_after_read and not df_sheet.empty:
                        # For 同花顺 sheets, the row after the header is '指标ID', skip it.
                        print(f"     Skipping the row after header (Index 0 in loaded data for 同花顺).")
...
if __name__ == "__main__":
    ...
    target_file_path = os.path.join('data', '经济数据库0424_带数据源标志.xlsx')
