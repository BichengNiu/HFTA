...
def display_weekly_tab(st, session_state):
    ...
    # Check for weekly_df specifically for this tab's operation
    if not hasattr(session_state, 'weekly_df') or session_state.weekly_df.empty:
        st.info("周度数据尚未加载或为空，请返回'数据概览'模块上传数据。") # Updated prompt
        return

    # Use the pre-populated weekly industries list
    weekly_industries = session_state.get('weekly_industries', [])
    ...
    st.markdown("--- ") # Divider AFTER Middle Section Title (ADDED)
    if not session_state.weekly_df.empty:
        ...
        if selected_industry_w_clean:
            ...
            if selected_industry_w_clean not in session_state.weekly_summary_cache:
                with st.spinner(f"正在计算 '{selected_industry_w_clean}' 的周度摘要..."):
                    ...
                    if industry_indicator_cols:
                        ...
                        session_state.weekly_summary_cache[selected_industry_w_clean] = summary_table
                    else:
                        session_state.weekly_summary_cache[selected_industry_w_clean] = pd.DataFrame()

            summary_table = session_state.weekly_summary_cache[selected_industry_w_clean]
            ...
            if not industry_indicators:
                ...
                for indicator in industry_indicators:
                    indicator_series = session_state.weekly_df[indicator].dropna()
