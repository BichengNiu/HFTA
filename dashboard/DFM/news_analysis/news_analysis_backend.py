...
try:
    ...
    DFM_NEWS_OUTPUT_DIR = dfm_config.DFM_NEWS_OUTPUT_DIR
    ensure_output_dirs = dfm_config.ensure_output_dirs
    _CONFIG_AVAILABLE = True
    print("[Backend] Successfully imported DFM config")
except ImportError as e:
    print(f"[Backend] 无法导入DFM配置: {e}")
    _CONFIG_AVAILABLE = False
    DFM_NEWS_OUTPUT_DIR = None  # 将使用系统临时目录
    print("Warning: 无法导入统一配置，将使用系统临时目录")
...
def execute_news_analysis(
    ...
    # ✅ 修改：使用系统临时目录而不是项目内固定目录
    if base_workspace_dir is None:
        if _CONFIG_AVAILABLE:
            # 如果配置可用，使用配置的输出目录
            ensure_output_dirs()
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            base_workspace_dir = os.path.join(DFM_NEWS_OUTPUT_DIR, f"analysis_{timestamp}")
            os.makedirs(base_workspace_dir, exist_ok=True)
            print(f"[Backend] 使用配置的输出目录: {base_workspace_dir}")
        else:
            # 配置不可用时，创建临时目录用于此次分析
