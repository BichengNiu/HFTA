...
def prepare_data_from_ui_input(
    ...
    try:
        ...
        if selected_variables:
            ...
            print(f"    数据中的列名: {list(input_df.columns)}")
            ...
            for ui_var in selected_variables:
                if ui_var == target_variable:
                    continue  # 跳过目标变量

                # 尝试精确匹配
                if ui_var in available_columns:
                    mapped_vars.append(ui_var)
                    print(f"    ✅ 精确匹配: '{ui_var}'")
                else:
                    # 尝试大小写不敏感匹配
                    ...
                    for col in available_columns:
                        if col.lower().strip() == ui_var_lower:
                            mapped_vars.append(col)
                            print(f"    🔧 变量名映射: '{ui_var}' -> '{col}'")
                            found = True
                            break
                    ...
                    if not found:
                        ...
                        for col in available_columns:
                            ...
                            if ui_var_punct_normalized == col_punct_normalized:
                                mapped_vars.append(col)
                                print(f"    🔧 变量名映射 (标点符号标准化): '{ui_var}' -> '{col}'")
                                found = True
                                break
                    ...
                    if not found:
                        ...
                        for col in available_columns:
                            ...
                            if ui_var_normalized == col_normalized:
                                mapped_vars.append(col)
                                print(f"    🔧 变量名映射 (Unicode标准化): '{ui_var}' -> '{col}'")
                                found = True
                                break

                    if not found:
                        # 🔥 新增：尝试Unicode + 标点符号双重标准化匹配
                        ...
                        for col in available_columns:
                            ...
                            if ui_var_full_normalized == col_full_normalized:
                                mapped_vars.append(col)
                                print(f"    🔧 变量名映射 (完全标准化): '{ui_var}' -> '{col}'")
                                found = True
                                break
                    ...
                    if not found:
                        ...
                        for col in available_columns:
                            ...
                            if ui_var_lower in col_lower or col_lower in ui_var_lower:
                                mapped_vars.append(col)
                                print(f"    🔧 变量名映射 (部分匹配): '{ui_var}' -> '{col}'")
                                found = True
                                break

                    if not found:
                        print(f"    ⚠️ 警告: 未找到变量 '{ui_var}' 的匹配列")

            final_variables.extend(mapped_vars)
            print(f"  🔍 [UI数据准备] 映射结果: {len(mapped_vars)} 个预测变量")
        else:
            # 如果没有选择变量，使用所有可用变量
            final_variables = list(input_df.columns)

        print(f"  最终变量列表: {len(final_variables)} 个变量")

        # 3. 过滤数据
        filtered_data = input_df[final_variables].copy()

        # 🔥 修复：检查并去除重复列
        print(f"  🔧 数据去重前形状: {filtered_data.shape}")
        print(f"  🔧 数据去重前列数: {len(filtered_data.columns)}")

        # 检查重复列
        duplicate_cols = filtered_data.columns.duplicated()
        if duplicate_cols.any():
            duplicate_col_names = filtered_data.columns[duplicate_cols].tolist()
            print(f"  🔧 发现重复列: {duplicate_col_names}")
            # 去除重复列，保留第一个
            filtered_data = filtered_data.loc[:, ~duplicate_cols]
            print(f"  🔧 去重后形状: {filtered_data.shape}")

            # 更新final_variables列表
            final_variables = list(filtered_data.columns)
            print(f"  🔧 更新后变量列表: {len(final_variables)} 个变量")
        else:
            print(f"  ✅ 未发现重复列")

        # 4. 🔥 修复：区分训练数据范围和nowcast计算数据范围
        # 训练数据范围：用于模型训练，受validation_end_date限制
        # nowcast计算数据范围：用于计算nowcast，应该包含最新的2025年数据

        print(f"  🔥 数据范围处理:")
        print(f"    原始数据范围: {filtered_data.index.min()} 到 {filtered_data.index.max()}")
        print(f"    训练开始日期: {training_start_date}")
        print(f"    验证结束日期: {validation_end_date}")

        # 保存完整数据用于nowcast计算
        full_data_for_nowcast = filtered_data.copy()
        ...
        if training_start_date or validation_end_date:
            try:
                if training_start_date:
                    ...
                    print(f"    训练开始日期过滤后: {filtered_data.shape}")

                if validation_end_date:
                    end_date = pd.to_datetime(validation_end_date)
                    filtered_data = filtered_data[filtered_data.index <= end_date]
                    print(f"    验证结束日期过滤后: {filtered_data.shape}")
                    print(f"    🔥 关键：训练数据被限制到 {end_date}，但nowcast计算将使用完整数据到 {full_data_for_nowcast.index.max()}")

                print(f"  最终训练数据形状: {filtered_data.shape}")
                print(f"  完整nowcast数据形状: {full_data_for_nowcast.shape}")

            except Exception as e:
                print(f"  日期过滤警告: {e}")
                full_data_for_nowcast = filtered_data.copy()  # 如果过滤失败，使用相同数据
        ...
        if not skip_stationarity_check:
            print(f"  开始平稳性检查...")
            # 跳过目标变量的平稳性检查
            skip_cols = {target_variable}

            transformed_data, transform_log, removed_cols_info = _ensure_stationarity(
                filtered_data,
                skip_cols=skip_cols,
                adf_p_threshold=0.05
            )

            transform_details = transform_log
            removed_variables_log = removed_cols_info

            print(f"  平稳性检查完成，最终数据形状: {transformed_data.shape}")
        else:
            print(f"  跳过平稳性检查")
        ...
        if target_variable not in transformed_data.columns:
            raise ValueError(f"处理后目标变量 '{target_variable}' 丢失")

        print(f"--- [UI数据准备] 完成，输出数据形状: {transformed_data.shape} ---")

        return transformed_data, transform_details, removed_variables_log
