...
### 统一输出目录
所有DFM相关输出现在统一存储在 `dashboard/DFM/outputs/` 下：

```
dashboard/DFM/outputs/
├── train_results/          # 训练结果
│   └── run_{timestamp}/    # 每次训练的结果
│       ├── models/         # 模型文件 (.pkl, .joblib)
│       ├── data/          # 数据文件 (.csv)
│       ├── plots/         # 图表文件 (.png)
│       └── reports/       # 报告文件 (.xlsx)
...
dashboard/DFM/
├── config.py                 # 统一配置文件
├── migrate_outputs.py        # 输出迁移工具
├── output_analysis_report.md # 输出分析报告
├── README.md                 # 本文档
├── data_prep/               # 数据预处理模块
│   ├── data_preparation.py
│   └── data_prep_ui.py
...
如果您有旧版本的输出文件需要迁移到新的统一目录结构，可以使用迁移工具：

```bash
# 扫描旧文件
python -m dashboard.DFM.migrate_outputs --scan-only

# 模拟迁移（不实际移动文件）
python -m dashboard.DFM.migrate_outputs --dry-run

# 执行实际迁移
python -m dashboard.DFM.migrate_outputs

# 迁移并清理旧目录
python -m dashboard.DFM.migrate_outputs --cleanup
```
